...
while count < 30:
    ...
    for i in agentJson:
        if (i["type"] != "collaborator") and (i["occasional"] != True) and (i["contact"]["active"] != False) and (i["deactivated"] != True):
            rowCount = rowCount + 1
            if i["contact"]["last_login_at"] is not None and (i["contact"]["email"] not in omittedAddresses):
                if datetime.now() - datetime.strptime(i["contact"]["last_login_at"], "%Y-%m-%dT%H:%M:%SZ") > timedelta(days=delta):
                    agentCount = agentCount + 1
                    print(agentCount,"name:",i["contact"]["name"],":email:",i["contact"]["email"],":isactive:",i["contact"]["active"],":last login:",i["contact"]["last_login_at"])
    count = count + 1 